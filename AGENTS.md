...
- **NEVER treat regex `.` as "any character"** — `.` excludes `\n` (and `\r` in many engines). A native shortcut like "if slash, accept remainder" silently widens the accepted set. After any delimiter, scan remaining characters and reject line terminators.
