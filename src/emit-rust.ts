  ...
  for (const action of actions) {
    switch (action.type) {
      case 'check_content_min_length':
        lines.push(`        if content.chars().count() < ${action.min} {`);
      ...
      case 'check_content_max_length':
        lines.push(`        if content.chars().count() > ${action.max} {`);
...
function emitRustFile(
  ...
  const eventDispatchCode = emitEventDispatchRust(constrainedKinds, contentPlans, helpers, adapter, api);
